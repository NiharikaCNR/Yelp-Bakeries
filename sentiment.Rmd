...
sentiment_store<-sentiment_data%>%
  inner_join(get_sentiments("afinn")) %>%
  count(business_id,word,value) %>% 
   group_by(business_id) %>%
   summarize(average_value = mean(value, na.rm = TRUE)) %>%
   mutate(sentiment_label = ifelse(average_value > 1.5, "excellent",
                                   ifelse(average_value > 0.7, "good", 
                                   ifelse(average_value > 0, "below average","need work"))))

write_csv(sentiment_store, "sentiment_store.csv")



mean(sentiment_store$average_value)
...
sentiment_store_close<-sentiment_data_close%>%
  inner_join(get_sentiments("afinn")) %>%
  count(business_id,word,value)%>%
  group_by(business_id) %>%
  summarize(average_value = mean(value, na.rm = TRUE))
...
mean(sentiment_store_close$average_value)
```
...
sentiment_store_open<-sentiment_data_open%>%
  inner_join(get_sentiments("afinn")) %>%
  count(business_id,word,value)%>%
  group_by(business_id) %>%
  summarize(average_value = mean(value, na.rm = TRUE)) %>% 
...
mean(sentiment_store_open$average_value)
```